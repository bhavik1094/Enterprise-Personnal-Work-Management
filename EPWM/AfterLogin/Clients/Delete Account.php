...
<html>
    ...
    <body style="background: #f7f8f9;">
        ...
        if (isset($_POST['btnDelAcc'])) {
            $sql = mysqli_query($con, "select * from clientmaster where Client_email='$userClient'");
            ...
            $reasonInsert = mysqli_query($con, "insert into deleted_acc_data values('$name','$clientId','$reason','$details','$person_type')");
            if ($reasonInsert) {
                $reasonDelete = mysqli_query($con, "DELETE FROM clientmaster WHERE client_name='$name'");
                if ($reasonDelete) {
