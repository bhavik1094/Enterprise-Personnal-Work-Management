...
<html>
    ...
    <body>
        ...
        <div class="pwdForm postProjectForm">
            <form method="post">
                <div class="form-group">
                    ...
                    <select name="projName" class="form-control" required="">
                        <option class="hidden"  selected disabled>Please select your current project</option>
                        <?php
                        $sql = mysqli_query($con, "select * from projectmaster where client_id='$client_id'");
                        $row = mysqli_num_rows($sql);
                        while ($row = mysqli_fetch_array($sql)) {
                            $project_name="<option value='".$row['proj_name']."'>".$row['proj_name']."</option>";
                            echo $project_name;
                ...
                <input type="submit" name="btnEditProject" value="Submit Project" class="btnSubmitProject"/>
            </form>
        </div>
        ...
        if (isset($_POST['btnEditProject'])) {
            ...
            $projDetails = $_REQUEST['projDetails'];
            ...
            $editProject = mysqli_query($con, "update projectmaster set proj_detail='$projDetails', proj_skills='$skills2', start_date='$startDate', end_date='$endDate', proj_type='$projType', currency_type='$currencyType', proj_amount='$amount' where project_id='$proj_id' and proj_name='$projName'");
            if ($editProject) {
                echo "<script>alert('Your Project is updated')</script>";
