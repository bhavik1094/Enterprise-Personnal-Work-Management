...
<html>
    ...
    <body>
                ...
                <div class="bank-form-content">
                    <form method="post">
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <input type="text" name="clientName" class="form-control" placeholder="Your Name in Bank *" value=""/>
                                </div>
                                <div class="form-group">
        ...
        <?php
        $sessionEmail = $_SESSION['login_client'];
        if (isset($_POST['btnLeaveSubmit'])) {
            $bankName = $_REQUEST['bankName'];
            $bankIFSC = $_REQUEST['bankIfsc'];
            $bankClientName = $_REQUEST['clientName'];
            $bankAccNo = $_REQUEST['bankAccountNo'];
            $person_type="Client";
            $res = mysqli_query($con, "select client_id from clientmaster where client_email='$userClient'");

            while ($row = mysqli_fetch_assoc($res)) {
                $clientId = $row['client_id'];
            }
                    $sql = mysqli_query($con, "insert into bank_details values ('$clientId','$bankName','$bankIFSC','$bankClientName','$bankAccNo','$person_type')");
                    if ($sql) {
